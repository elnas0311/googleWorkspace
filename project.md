...
## 背景
### 世界の現状
2025年、独立行政法人情報処理推進機構（IPA）は「情報セキュリティ10大脅威 2025」を公表し、その中で新たに「地政学的リスクに起因するサイバー攻撃」が選出されました。IPAのプレスリリースによると、これは国家や地域ごとの政治・軍事的リスクに基づき、他国が巨額の予算を投入してサイバー攻撃を仕掛ける可能性を示唆するものであり、従来の個人向け情報漏えい対策中心のセキュリティ対策とは異なり、組織全体の資産（情報、金銭、不動産、知的財産など）の管理・保護が求められる状況になっています

### これまでのセキュリティ対策と今後の課題
従来のセキュリティ対策は、主に個人のインターネット利用やパソコンを対象に、情報漏洩防止に注力してきました。しかし、サイバー犯罪の組織化と国家レベルでの高度化が進む中、攻撃手法は人的ミスや隙を突くだけでなく、金銭的な詐欺、不動産や知的財産の流出といったリスクにも発展していくと予測されます。今後は「誰がどの情報にアクセスできるか」「誰がどの情報を保有しているか」を厳密に管理し、事前にリスクを排除する体制が求められます。[METI.GO.JP](https://www.meti.go.jp/meti_lib/report/2023FY/000408.pdf)


### 既存ドメインのリスク
sawarise.co.jp ドメインは、すでに大量の迷惑メールが届いているなど、スパムや不正利用のリスクが顕在化しています。攻撃者がこのドメインを踏み台に悪意あるメールを送信する可能性もあり、ブランドの信用失墜やセキュリティインシデントに発展する恐れがありますし、既にセキュリティインシデントが発生している可能性もあります。

### 新しいドメインへの移行とアイデンティティ管理
GoogleWorkspaceで利用しているドメインである、sawarise.comをアカウント登録し
...
## 目標
- **情報セキュリティ:**
  安全に業務を遂行してもらうためにアカウント情報の管理を行いインシデント発生のリスク低下と発生時の早急な対応が行えるようにする。
- **業務効率化:**  
...
- **メールサービス統合:**  
  sawarise.co.jpで運用していたメールサービスを2026年4月1日付で停止（2026年3月でサイボウズを停止）し、以降はsawarise.comドメインのGmailで統一運用する。
...
- **プロダクトマネージャー:** [谷口義明]
- **プロジェクトマネージャー:** [猿渡宣弘]
- **技術担当:** Google Workspace移行担当、Rakumo連携担当、チャットボット開発担当 [フューテックセンター]
- **事業所代表者:** 各事業所のシステム管理者および運用担当 [各事業所からDXプロジェクトへアサイン]
- **サポートチーム:** 移行後サポート担当（専任サポートチーム）[猿渡宣弘、松山祐一]
...
- **テスト環境構築＆初期移行 (14日):**  
  ...
  FCのスペシャリストを対象に、Gmail、Googleカレンダーへ移行
